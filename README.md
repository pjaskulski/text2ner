...
- `TEXT2NER_WIKIBASE_ENTITY_CACHE_DB_PATH` - ścieżka trwałego cache SQLite dla danych `wbgetentities`, domyślnie `/tmp/text2ner_wikibase_entity_cache.sqlite3`,
- `TEXT2NER_WIKIBASE_ENTITY_CACHE_TTL_SECONDS` - czas ważności pełnych encji w cache `wbgetentities`, domyślnie 90 dni; wartość `0` wyłącza trwały cache,
- `TEXT2NER_WIKIBASE_ENTITY_CACHE_MISSING_TTL_SECONDS` - czas ważności pustych/brakujących encji w cache `wbgetentities`, domyślnie 1 dzień,
- `TEXT2NER_WIKIBASE_ENTITY_CACHE_MAX_BYTES` - maksymalny rozmiar pojedynczego wpisu zapisywanego do trwałego cache `wbgetentities`, domyślnie 500 KB; większe encje pozostają tylko w cache RAM bieżącego procesu, a wartość `0` wyłącza limit rozmiaru,
